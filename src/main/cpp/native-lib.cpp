...
Java_com_example_zipper_MainActivity_archiveAndSecure(
        JNIEnv* env,
        jobject /* this */,
        jstring inputString, jstring save_to) {
    ...
    std::string path_save_to(native_save_to);
    env->ReleaseStringUTFChars(save_to, native_save_to);
    ...
    try {
        utils::write_int_data_into_bin_file(ciphertext, path_save_to.c_str());
    } catch(std::string& err) {
        LOGD("%s", err.c_str());
    }

    return env->NewStringUTF(path_save_to.c_str());
}
...
Java_com_example_zipper_MainActivity_unarchiveAndOpen(
    ...
    const char *native_path_to = env->GetStringUTFChars(path_to, 0);
