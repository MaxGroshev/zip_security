...
with open('data/bin_dict_comp.txt') as file:
    for line in file:
...
ax.axis([0, max(num_of_elems) + max(num_of_elems)* 0.1, 10, 300])
...
ax.set_title ("\n".join(wrap('Pic. 5. Comparing ratio of lzw and zstd on binary files', 60)), loc = 'center', size = 30)
...
fig.savefig('pic/bin_dict_comp.png')
