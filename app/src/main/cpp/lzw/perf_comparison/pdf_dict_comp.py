...
with open('data/pdf_dict_comp.txt') as file:
    for line in file:
...
ax.axis([0, max(num_of_elems) + max(num_of_elems)* 0.1, 80, 200])
...
ax.set_title ("\n".join(wrap('Pic. 4. Comparing ratio of lzw and zstd on pdf', 60)), loc = 'center', size = 30)
...
fig.savefig('pic/pdf_dict_comp.png')
